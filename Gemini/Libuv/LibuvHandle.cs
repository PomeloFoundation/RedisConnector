...
{
    unsafe public abstract class LibuvHandle : IDisposable
    {
        IntPtr _handle;
        LibuvLoop _loop;
        ...
        protected LibuvHandle(LibuvLoop loop, HandleType handleType)
        {
        ...
        public bool IsDisposing
        {
            get
            {
                ...
                return LibuvManaged.uv_is_closing(_handle) != 0;
            }
        }

        static LibuvManaged.close_callback CloseCallback = OnClose;

        static void OnClose(IntPtr handlePointer)
        {
            var handle = LibuvHandle.As<LibuvHandle>(handlePointer);
